...
import type { AgentAccount, A2APeer, ApiProviderConfig, AppSettings } from '@shared/types'
...
export function AppSettingsPanel({ onClose }: AppSettingsPanelProps): React.JSX.Element {
  ...
  const sections: SettingsSection[] = [
    { id: 'user', title: 'user', render: (c) => <UserSection settings={c.settings} update={c.update} /> },
...
function UserSection({ settings, update }: { settings: AppSettings; update: (p: Partial<AppSettings>) => Promise<void> }): React.JSX.Element {
  ...
  return (
    <div className="settings-section">
      ...
      <p className="app-settings-hint">Basic user profile. Cloud/account sign-in is added here when the cloud feature lands — this section is the extension point.</p>
    </div>
  )
}
