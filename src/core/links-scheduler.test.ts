...
describe('LinkScheduler', () => {
  ...
  it.each([{ auto: false }, { source: 'other' }])('invalidates queued work on an eligibility change %j', async (patch) => {
    const deps = makeDeps()
    const s = new LinkScheduler(deps.onRun, { gapMs: 100 })
    s.setLinks([link()])
    s.markDirty('src-1')
    s.setLinks([link(patch)])
    await vi.advanceTimersByTimeAsync(200)
    expect(deps.runs).toEqual([])
    s.dispose()
  })

  it('retains dirty sources across intermediate metadata updates', async () => {
    const deps = makeDeps()
    const s = new LinkScheduler(deps.onRun, { gapMs: 100 })
    s.markDirty('src-1')
    s.setLinks([])
    s.setLinks([link({ id: 'other', source: 'other' })])
    s.setLinks([link()])
    await vi.advanceTimersByTimeAsync(200)
    expect(deps.runs).toEqual(['lk-1'])
    s.dispose()
